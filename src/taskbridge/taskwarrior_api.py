...
class TaskWarriorAPI:
    ...
    def _verify_taskwarrior(self) -> None:
        ...
        try:
            result = subprocess.run(
                [self.task_cmd, "version"], capture_output=True, text=True, check=True
            )
    ...
    def _run_task_command(self, args: list[str], input_data: str = None) -> str:
        ...
        try:
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, input=input_data, check=True
            )
    ...
    def get_task_by_uuid(self, task_uuid: str) -> TaskWarriorTask | None:
