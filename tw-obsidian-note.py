...
def get_obsidian_config():
    ...
    if yaml_config.exists():
        ...
        print_status('red', "❌ Taskbridge config not found. Run 'taskbridge config-obsidian' first.")
        sys.exit(1)
    ...
    if not vault_path:
        print_status('red', "❌ Obsidian vault path not configured. Run 'taskbridge config-obsidian' first.")
        sys.exit(1)
...
def create_obsidian_note(vault_path: str, project_name: str, task_title: str, task_uuid: str) -> Path:
...
def annotate_task(task_uuid: str, project_name: str, task_title: str, vault_name: str):
    ...
    if not existing_link:
        try:
            subprocess.run(
                ['task', task_uuid, 'annotate', obsidian_url],
                check=True
            )
